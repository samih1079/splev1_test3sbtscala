...
import SPLETools.{allProgsDistinctPairs, classesResEachProgPair, spark}
  ...
  def showClassesResultForEachProgPair(allClassRes:DataFrame): Unit ={
    ...
    for (i <- 1 to iterations) {
      ...
      tmpVals.foreach(r => {
        ...
        if (isfirst) {
          isfirst=false
          dataDF = towProgsRes.select(towProgsRes.columns.map(c => towProgsRes.col(c).cast("string")): _*)
          import scala.collection.JavaConverters._
          val headerDF = spark.createDataFrame(List(Row.fromSeq(dataDF.columns.toSeq)).asJava, dataDF.schema)
          headerDF.union(dataDF).write.mode(SaveMode.Overwrite).option("header", "false").csv(outputDir)
